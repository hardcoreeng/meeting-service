...
class Request<P extends any[], M extends string = string> {
  id?: ReqId
  method: M
  params: P
...
// For now we use Status, but in future that be moved to platform service that
// will be responsible for security, auth and so on, meeting service would return regular
// errors
interface Status {
  severity: 'ERROR'
  code: 'UnknownError'
